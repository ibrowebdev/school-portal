...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        <div class="p-6">
            ...
            <form action="{{ route('invoice/update/save') }}" class="x-submit space-y-8" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="lg:col-span-4 space-y-4">
                        <div>
                            ...
                            <select class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm select @error('full_name') border-red-500 @enderror" id="customer_name" name="customer_name">
                        ...
                        <div>
                            ...
                            <input class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" type="text" id="po_number" name="po_number" value="{{ $invoiceView->po_number }}">
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-5">
                        ...
                        <div class="bg-gray-50 rounded-xl p-5 border border-gray-200">
                            <div class="flex justify-between items-center mb-4 pb-4 border-b border-gray-200">
                                <span class="text-gray-700 font-medium">Invoice No. <a class="text-blue-600 hover:text-blue-800 ml-1 cursor-pointer">{{ $invoiceView->invoice_id }}</a></span>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm datetimepicker" type="text" name="due_date" value="{{ $invoiceView->due_date }}">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="bg-blue-50 rounded-xl p-5 border border-blue-100 h-full">
                            <div class="space-y-3 mb-4">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" id="enableTax" name="enable_tax" value="Enable tax" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" {{ !empty($invoiceView->enable_tax) ? 'checked' : '' }}>
                                    <span class="text-sm font-medium text-gray-700">Enable tax</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" id="chkYes" name="recurring_incoice" value="Recurring Invoice" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" {{ !empty($invoiceView->recurring_incoice) ? 'checked' : '' }}>
                                    <span class="text-sm font-medium text-gray-700">Recurring Invoice</span>
                                </label>
                            </div>
                            
                            <div id="show-invoices" class="{{ empty($invoiceView->recurring_incoice) ? 'hidden' : '' }}">
                                <div class="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-blue-200">
                                    <div>
                                        <select class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm select" id="by_month" name="by_month">
                                            <option selected>{{ $invoiceView->by_month ?? 'By month' }}</option>
                                    ...
                                    <div>
                                        <input class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" type="text" name="month" placeholder="Enter Months" value="{{ $invoiceView->month ?? '' }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-100">
                    <div>
                        ...
                        <textarea rows="3" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" name="invoice_from">{{ $invoiceView->invoice_from }}</textarea>
                    </div>
                    <div>
                        <strong class="block text-sm font-medium text-gray-700 mb-2">Invoice To</strong>
                        <textarea rows="3" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" name="invoice_to">{{ $invoiceView->invoice_to }}</textarea>
                    </div>
                </div>
                        ...
                        <table class="w-full text-sm text-left add-table-items" id="invoice-add-table">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($invoiceDetails as $key => $value)
                                    <tr class="add-row">
                                        <td hidden><input type="text" name="id[]" value="{{ $value->id }}"></td>
                                        <td class="p-2"><input type="text" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" name="items[]" value="{{ $value->items }}"></td>
                                        <td class="p-2"><input type="text" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" name="category[]" value="{{ $value->category }}"></td>
                                        <td class="p-2"><input type="text" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" name="quantity[]" oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" value="{{ $value->quantity }}"></td>
                                        <td class="p-2"><input type="text" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm price" name="price[]" oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" value="{{ $value->price }}"></td>
                                        <td class="p-2"><input type="text" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm amount" name="amount[]" oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" value="{{ $value->amount }}" ></td>
                                        <td class="p-2"><input type="text" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm discount" name="discount[]" oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" value="{{ $value->discount }}"></td>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-6 border-t border-gray-100">
                    ...
                    <div class="lg:col-span-7 space-y-6">
                        ...
                        <div id="{{ !empty($invoiceView->account_holder_name) ? 'bank-details-em' : 'bank-details' }}" class="bg-gray-50 p-5 rounded-xl border border-gray-200 {{ empty($invoiceView->account_holder_name) ? 'hidden' : '' }}">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Account Holder Name</label>
                                    <input type="text" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" id="auto_account_holder_name" name="account_holder_name" placeholder="Add Name" value="{{ $invoiceView->account_holder_name }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Bank name</label>
                                    <input type="text" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" id="auto_bank_name" name="bank_name" placeholder="Add Bank name" value="{{ $invoiceView->bank_name }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">IFSC Code</label>
                                    <input type="text" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" id="auto_ifsc_code" name="ifsc_code" placeholder="IFSC Code" value="{{ $invoiceView->ifsc_code }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Account Number</label>
                                    <input type="text" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" id="auto_account_number" name="account_number" placeholder="Account Number" value="{{ $invoiceView->account_number }}">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="lg:col-span-5 space-y-6">
                        <div class="bg-gray-50 rounded-xl p-6 border border-gray-200">
                            ...
                            <div class="space-y-3 mb-4">
                                <div class="flex justify-between items-center text-sm">
                                    ...
                                    <span class="font-medium text-gray-800">${{ $invoiceView->taxable_amount }}</span>
                                ...
                                <div class="flex justify-between items-center text-sm">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <input type="checkbox" name="round_off" id="status_1" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500" value="{{ !empty($invoiceView->round_off) ? $invoiceView->round_off : '54' }}" {{ !empty($invoiceView->round_off) ? 'checked' : '' }}>
                                    </div>
                                    <span class="font-medium text-gray-800">${{ !empty($invoiceView->round_off) ? $invoiceView->round_off : '54' }}</span>
                                </div>
                            </div>

                            <div class="links-info-one space-y-2 mb-3">
                                    ...
                                    @foreach($AdditionalCharges as $key => $value)
                                        <div class="links-cont flex justify-between items-center bg-gray-50 p-2 rounded border border-gray-200 mt-2">
                                            <div class="service-amount flex items-center justify-between w-full">
                                                ...
                                                <span class="font-medium text-gray-700">${{ $value->service_charge }}</span>
                            ...
                            <div class="links-info-discount space-y-2 mb-3">
                                    ...
                                    @foreach($InvoiceDiscount as $key => $value)
                                        <div class="links-cont-discount flex justify-between items-center bg-gray-50 p-2 rounded border border-gray-200 mt-2">
                                            <div class="service-amount flex items-center justify-between w-full">
                                                ...
                                                <span class="font-medium text-gray-700">${{ $value->offer_new }}%</span>
                            ...
                            <div class="flex justify-between items-center pt-2">
                                ...
                                <h4 class="text-xl font-bold text-blue-600" id="total_amount">$<span class="total_amount" name="total_amount">{{ $invoiceView->total_amount }}</span></h4>
                                <input type="hidden" id="total_amounts" name="total_amount" value="{{ $invoiceView->total_amount }}">
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Update Sign</label>
                                ...
                                @if(!empty($invoiceView->upload_sign))
                                    <div class="mt-3">
                                        <img class="max-w-[150px] rounded border border-gray-200" src="{{ asset($invoiceView->upload_sign) }}" alt="sign">
                                    </div>
                                @endif
                                <input type="hidden" name="upload_sign_unlink" value="{{ $invoiceView->upload_sign }}">
                            </div>
                            <div>
                                <input type="text" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" name="name_of_the_signatuaory" placeholder="Name of the Signatuaory" value="{{ $invoiceView->name_of_the_signatuaory }}">
                            </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-xl border-0 shadow-lg">
            <div class="p-6 text-center">
                ...
                <div id="form-errors-container-delete" class="hidden alert alert-danger">
                    <ul id="form-errors-list-delete" class="mb-0"></ul>
                </div>
                <form action="{{ route('invoices.destroy', $record->id ?? $invoice->id ?? 0) }}" method="POST" class="x-submit" data-then="reload">
                    ...
                    <div class="flex gap-3 justify-center">
                        <button type="submit" class="px-6 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 transition font-medium">Delete</button>
...
@section('script')
        ...
        $(function() {
            $("#save-item").click(function() {
                if (!isNaN($("#account_holder_name").val())) {
                    ...
                    $('#bank-details').removeClass('hidden').show();
    ...
    <script>
        ...
        $(document).on("click", ".add-btn", function() {
            ...
            '<tr class="add-row">' +
                '<td class="p-2">' + '<input type="text" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" name="items[]">' + '</td>' +
